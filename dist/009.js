"use strict";
//Favorite Number: Store your favorite number 5  in a variable. Then, using that variable, create a message that reveals your favorite number. Print that message.
Object.defineProperty(exports, "__esModule", { value: true });
// Store your favorite number in a variable
let favoriteNumber = 5;
// Create a message revealing your favorite number
let message = `My favorite number is ${favoriteNumber}.`;
// Print the message
console.log(message);
